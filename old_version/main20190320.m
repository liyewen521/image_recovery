...
image_data_median_vector = [reshape(image_data_median',1,table_height*table_width);1:table_height*table_width];
...
for i = 1 : table_height*table_width
    ...
    for k = number_of_pixel_bit : -1 : number_of_pixel_bit-4
        watermark_map_vector(i) = bitset( watermark_map_vector(i) , k+4 , bitget(image_data_median_vector(1,logistic_sequence_one(1,i)),k) );
        watermark_map_vector(i) = bitset( watermark_map_vector(i) , k-1 , bitget(image_data_median_vector(1,logistic_sequence_two(1,i)),k) );       
    end
...
% image_data_tampered(1:128,1:128)= 0; image_data_tampered(129:256,129:256)= 0;  % 指定损坏区域 50% ,反斜杠
% for i= 256 : -1 : 1
    ...
    for j = 1 : 1 : table_width
        if block_valid_or_invalid(i,j) == 1
            ...
            if block_valid_or_invalid(block_row,block_col) == 1
                original_value = floor(double(image_data_median(block_row,block_col))/8)*8;
