...
for i = 1 : table_height
    for j = 1 : table_width
        pre_embed_recover_data_256(i*block_height,j*block_width) = image_data_avg(i,j);
        pre_embed_recover_data_256(i*block_height-1,j*block_width) = image_data_avg(i,j);
        pre_embed_recover_data_256(i*block_height,j*block_width-1) = image_data_avg(i,j);
        pre_embed_recover_data_256(i*block_height-1,j*block_width-1) = image_data_avg(i,j);
    end
end
    ...
    for j = 1 : 1 : table_width
        if block_valid_or_invalid(i,j) == 1
            ...
            if block_valid_or_invalid(block_row,block_col) == 1
                original_value = floor(double(image_data_avg(block_row,block_col))/8)*8;
