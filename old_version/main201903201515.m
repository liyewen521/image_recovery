% 恢复信息从平均值，中值，左中值，右中值里面选
% 编程小建议：尽量用double型变量来进行运算。因为uint8型变量当相减的结果小于
% 0就会出错，虽然int8不会，但是还是挺麻烦的。uint8在进行取整取余操作时候容易
% 出意想不到的问题
...
% 求各个块最佳的像素值
[pre_embed_recover_data,best_method,method_percentage] = find_best_method(image_data,table_height,table_width,block_height,block_width);
